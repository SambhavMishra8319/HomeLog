...
export default function Tenants({
  ...
  const filtered = tenants.filter((t) =>
    `${t.name} ${t.phone} ${t.room} ${t.payStatus} ${t.occupation}`
      .toLowerCase()
    ...
    <section className="panel">
      <div className="section-head">
        <div>
          <h3>Kirayadar CRM</h3>
          <p className="muted">
            Profiles, deposits, dues and agreement tracking.
          </p>
        </div>
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Search Kirayadar..."
        />
      </div>
      <form className="form" onSubmit={onAdd}>
        ...
        <input name="name" placeholder="Kirayadar name" required />
      ...
      <Table
        ...
        rows={filtered.map((t) => [
          ...
          t.documents,
          <button className="ghost" onClick={() => onRemove(t.id)}>
            Delete
          </button>,
        ])}
      />
